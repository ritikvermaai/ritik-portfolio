  ...
  return <>
    ...
    <section className="hero container-wide" style={{'--mx':mouse.x,'--my':mouse.y}}>
      ...
      <div className="hero-copy">
        ...
        <p className="hero-lead"><span className="hero-role"><span className="hero-role-stage"><span className="hero-role-sizer" aria-hidden="true">{safeRoles.reduce((longest, role) => role.length > longest.length ? role : longest, '')}</span><AnimatePresence initial={false} mode="wait"><motion.span key={safeRoles[roleIndex]} className="hero-role-word" initial={{opacity:0,scaleX:.72,scaleY:.94,filter:'blur(4px)',x:-2}} animate={{opacity:1,scaleX:1,scaleY:1,filter:'blur(0px)',x:0}} exit={{opacity:0,scaleX:.72,scaleY:.94,filter:'blur(4px)',x:3}} transition={{duration:.42,ease:[.22,1,.36,1]}}>{safeRoles[roleIndex]}</motion.span></AnimatePresence></span><span className="hero-role-cursor" aria-hidden="true" /></span><span className="hero-dot"> · </span>{content.hero?.tagline}</p>
...
function ProjectCard({project,onOpen,tilt=false}){const image=(project.images?.[0]||project.imageUrl||ASSET('img1.png'));const navigate=useNavigate();const open=()=>{api.send('/api/analytics/event','POST',{type:'project',id:project._id,name:project.title}).catch(()=>{});if(onOpen) onOpen(project); else navigate(`/projects/${project._id}`)};return <motion.article className={`project-card ${tilt ? 'tilt-card' : ''}`} whileHover={{y:-8}} onClick={open}><div className="project-media"><img src={image} alt={project.title} loading="lazy" /><span>{project.category||'Project'}</span>{project.featured&&<em>★ Featured</em>}</div><div className="project-body"><div className="project-top"><h3>{project.title}</h3><button className="icon-btn small" onClick={e=>{e.stopPropagation();open()}} aria-label="Open project showcase"><ExternalLink size={17}/></button></div><p>{project.description}</p><div className="tech-row">{(project.technologies||[]).slice(0,6).map(t=><span key={t}>{t}</span>)}</div><div className="project-links">{project.liveUrl&&<a href={project.liveUrl} onClick={e=>e.stopPropagation()} target="_blank" rel="noreferrer">Live <Globe2 size={14}/></a>}{project.githubUrl&&<a href={project.githubUrl} onClick={e=>e.stopPropagation()} target="_blank" rel="noreferrer">GitHub <GithubIcon size={14}/></a>}</div><div className="project-case-link">View full case study <ArrowRight size={14}/></div></div></motion.article>}

function Projects(){const [projects,setProjects]=useState([]);const [q,setQ]=useState('');const [cat,setCat]=useState('all');useEffect(()=>{ api.get('/api/projects').then(setProjects).catch(()=>{}); },[]); const cats=['all',...new Set(projects.map(p=>p.category).filter(Boolean))];const filtered=projects.filter(p=>`${p.title} ${p.description} ${p.overview||''} ${(p.technologies||[]).join(' ')}`.toLowerCase().includes(q.toLowerCase())&&(cat==='all'||p.category===cat));return <Page eyebrow="Portfolio" title="Things I've built" subtitle="Explore the projects, the decisions behind them, and the problems each one was built to solve."><div className="projects-intro glass"><div><span className="eyebrow">Built, tested, improved</span><h2>Work that tells a story.</h2><p>Open any project to see the stack, screenshots, approach, challenges, and results in a focused case-study experience.</p></div><div className="projects-intro-stats"><div><strong>{filtered.length}</strong><span>Projects shown</span></div><div><strong>{new Set(projects.flatMap(p=>p.technologies||[])).size}</strong><span>Technologies</span></div></div></div><div className="toolbar glass"><div className="search"><Search size={17}/><input value={q} onChange={e=>setQ(e.target.value)} placeholder="Search my projects..."/></div><div className="filter-pills">{cats.map(c=><button key={c} className={cat===c?'active':''} onClick={()=>setCat(c)}>{c}</button>)}</div></div><div className="project-grid project-grid-large">{filtered.map(p=><ProjectCard key={p._id} project={p}/>)}</div>{!filtered.length&&<EmptyState icon={FolderKanban} text="No projects match that search."/>}</Page>}

function ProjectDetail({project,onClose}){const image=(project.images?.[0]||project.imageUrl||ASSET('img1.png'));const navigate=useNavigate();return <Modal onClose={onClose} title={project.title}><div className="project-detail"><img src={image} alt={project.title}/><div className="project-detail-copy"><span className="eyebrow">{project.category||'Project'}</span><p>{project.description}</p><div className="tech-row">{(project.technologies||[]).map(t=><span key={t}>{t}</span>)}</div><div className="project-detail-actions"><button className="btn btn-primary" onClick={()=>{onClose();navigate(`/projects/${project._id}`)}}>Open full case study <ArrowRight size={15}/></button>{project.liveUrl&&<a className="btn btn-ghost" href={project.liveUrl} target="_blank" rel="noreferrer">Live demo <Globe2 size={15}/></a>}{project.githubUrl&&<a className="btn btn-ghost" href={project.githubUrl} target="_blank" rel="noreferrer">GitHub <GithubIcon size={15}/></a>}</div>{project.images?.length>1&&<div className="project-detail-gallery">{project.images.map((src,i)=><img key={i} src={src} alt={`${project.title} ${i+1}`}/>)}</div>}</div></div></Modal>}

function ProjectShowcase(){const {id}=useParams();const [projects,setProjects]=useState([]);const [loading,setLoading]=useState(true);const navigate=useNavigate();useEffect(()=>{let live=true;api.get('/api/projects').then(items=>{if(!live)return;setProjects(items||[]);const p=(items||[]).find(x=>String(x._id)===String(id));if(p)api.send('/api/analytics/event','POST',{type:'project',id:p._id,name:p.title}).catch(()=>{});}).catch(()=>{}).finally(()=>live&&setLoading(false));return()=>{live=false}},[id]);if(loading)return <Page eyebrow="Project showcase" title="Loading case study…" subtitle="Pulling together the project story and visuals."/>;const project=projects.find(x=>String(x._id)===String(id));if(!project)return <Page eyebrow="Project showcase" title="Project not found" subtitle="This project may have been removed or is no longer public."><button className="btn btn-primary" onClick={()=>navigate('/projects')}><ChevronLeft size={16}/>Back to projects</button></Page>;const images=project.images?.length?project.images:[project.imageUrl||ASSET('img1.png')];const overview=project.overview||project.description;const role=project.role||'Designed, developed, tested, and iterated on the project from idea to working experience.';const challenge=project.challenge||'Turn a practical idea into something clear, reliable, and useful while keeping the experience simple for the person using it.';const solution=project.solution||`Built the experience around ${((project.technologies||[]).slice(0,4).join(', ')||'the project stack')}, with focused UI, working interactions, and an implementation that can be improved over time.`;const results=project.results||'A working project that demonstrates the implementation, design decisions, and problem-solving behind the build.';const directVideo=project.videoUrl && /\.(mp4|webm|ogg)(\?|#|$)/i.test(project.videoUrl);return <Page eyebrow={`${project.category||'Project'} · Case study`} title={project.title} subtitle={project.description}><div className="showcase-hero glass"><div className="showcase-hero-copy"><div className="tech-row">{(project.technologies||[]).map(t=><span key={t}>{t}</span>)}</div><p>{overview}</p><div className="project-detail-actions"><button className="btn btn-ghost" onClick={()=>navigate('/projects')}><ChevronLeft size={15}/>All projects</button>{project.liveUrl&&<a className="btn btn-primary" href={project.liveUrl} target="_blank" rel="noreferrer">Live demo <Globe2 size={15}/></a>}{project.githubUrl&&<a className="btn btn-ghost" href={project.githubUrl} target="_blank" rel="noreferrer">Source code <GithubIcon size={15}/></a>}{project.videoUrl&&!directVideo&&<a className="btn btn-ghost" href={project.videoUrl} target="_blank" rel="noreferrer">Watch walkthrough <Play size={15}/></a>}</div></div><div className="showcase-cover"><img src={images[0]} alt={`${project.title} preview`} /></div></div><div className="showcase-layout"><div className="showcase-main"><section className="showcase-section glass"><span className="eyebrow">01 · Overview</span><h2>What I built</h2><p>{overview}</p></section><section className="showcase-section glass"><span className="eyebrow">02 · Challenge</span><h2>The problem to solve</h2><p>{challenge}</p></section><section className="showcase-section glass"><span className="eyebrow">03 · Solution</span><h2>How I approached it</h2><p>{solution}</p></section><section className="showcase-section glass"><span className="eyebrow">04 · Results</span><h2>What came out of it</h2><p>{results}</p></section>{project.videoUrl&&directVideo&&<section className="showcase-section glass"><span className="eyebrow">05 · Walkthrough</span><h2>See it in action</h2><video className="showcase-video" controls preload="metadata" src={project.videoUrl}/></section>}{images.length>0&&<section className="showcase-section glass"><span className="eyebrow">06 · Visuals</span><h2>Screenshots & details</h2><div className="showcase-gallery">{images.map((src,i)=><img key={src+i} src={src} alt={`${project.title} screenshot ${i+1}`} loading="lazy" />)}</div></section>}</div><aside className="showcase-side"><div className="glass showcase-meta"><span className="eyebrow">My role</span><p>{role}</p><div className="showcase-meta-row"><span>Category</span><strong>{project.category||'Project'}</strong></div><div className="showcase-meta-row"><span>Stack</span><strong>{(project.technologies||[]).length||'—'} technologies</strong></div></div><div className="glass showcase-next"><span className="eyebrow">Keep exploring</span><h3>More things I've built</h3><button className="btn btn-primary full" onClick={()=>navigate('/projects')}>Browse projects <ArrowRight size={15}/></button></div></aside></div></Page>}
...
function ProjectEditor({project,onClose,onSaved}){const [form,setForm]=useState({title:project?.title||'',category:project?.category||'Web Development',description:project?.description||'',overview:project?.overview||'',role:project?.role||'',challenge:project?.challenge||'',solution:project?.solution||'',results:project?.results||'',videoUrl:project?.videoUrl||'',technologies:(project?.technologies||[]).join(', '),liveUrl:project?.liveUrl||'',githubUrl:project?.githubUrl||'',featured:!!project?.featured});const [existing,setExisting]=useState(project?.images?.length?project.images:(project?.imageUrl?[project.imageUrl]:[]));const [files,setFiles]=useState([]);const [busy,setBusy]=useState(false);const submit=async e=>{e.preventDefault();setBusy(true);try{let uploaded=[];if(files.length){const fd=new FormData();files.forEach(f=>fd.append('images',f));const r=await api.form('/admin/projects/upload-images','POST',fd);uploaded=r.images||[]}const body={...form,technologies:form.technologies.split(',').map(s=>s.trim()).filter(Boolean),images:[...existing,...uploaded].slice(0,5)};if(project){await api.send(`/admin/projects/${project._id}`,'PUT',body)}else{await api.send('/admin/projects','POST',body)}onSaved()}catch(e){uiAlert(e.message)}finally{setBusy(false)}};return <Modal title={project?'Edit project':'Add project'} onClose={onClose}><form className="editor-form" onSubmit={submit}><div className="form-grid two"><Field label="Title"><input required value={form.title} onChange={e=>setForm({...form,title:e.target.value})}/></Field><Field label="Category"><input value={form.category} onChange={e=>setForm({...form,category:e.target.value})}/></Field></div><Field label="Short description"><textarea required value={form.description} onChange={e=>setForm({...form,description:e.target.value})}/></Field><Field label="Case-study overview"><textarea value={form.overview} onChange={e=>setForm({...form,overview:e.target.value})} placeholder="What is this project and why did you build it?"/></Field><Field label="My role"><textarea value={form.role} onChange={e=>setForm({...form,role:e.target.value})} placeholder="What did you personally design, build, test, or maintain?"/></Field><div className="form-grid two"><Field label="Challenge"><textarea value={form.challenge} onChange={e=>setForm({...form,challenge:e.target.value})} placeholder="What problem were you solving?"/></Field><Field label="Solution"><textarea value={form.solution} onChange={e=>setForm({...form,solution:e.target.value})} placeholder="How did you solve it?"/></Field></div><Field label="Results / impact"><textarea value={form.results} onChange={e=>setForm({...form,results:e.target.value})} placeholder="What worked, improved, or was achieved?"/></Field><div className="form-grid two"><Field label="Live URL"><input value={form.liveUrl} onChange={e=>setForm({...form,liveUrl:e.target.value})}/></Field><Field label="GitHub URL"><input value={form.githubUrl} onChange={e=>setForm({...form,githubUrl:e.target.value})}/></Field></div><Field label="Walkthrough video URL"><input value={form.videoUrl} onChange={e=>setForm({...form,videoUrl:e.target.value})} placeholder="Direct .mp4/.webm URL or any public video link"/></Field><Field label="Technologies (comma separated)"><input value={form.technologies} onChange={e=>setForm({...form,technologies:e.target.value})}/></Field><label className="checkbox"><input type="checkbox" checked={form.featured} onChange={e=>setForm({...form,featured:e.target.checked})}/><span>Featured project</span></label><Field label="Project screenshots (up to 5 new files)"><input type="file" accept="image/*" multiple onChange={e=>setFiles(Array.from(e.target.files||[]).slice(0,5-existing.length))}/></Field><div className="image-preview-row">{existing.map((url,i)=><div key={url} className="preview-thumb"><img src={url} alt="Project screenshot"/><button type="button" onClick={()=>setExisting(existing.filter((_,idx)=>idx!==i))}><X size={13}/></button></div>)}</div><small className="editor-help">These fields power the public project case-study page. Existing projects remain compatible; blank fields use tasteful fallback copy.</small><div className="modal-actions"><button type="button" className="btn btn-ghost" onClick={onClose}>Cancel</button><button className="btn btn-primary" disabled={busy}>{busy?<Loader2 className="spin"/>:<Save size={16}/>}Save project</button></div></form></Modal>}
